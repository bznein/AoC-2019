...
fn print_grid(grid: &[Vec<char>], pos: (usize, usize), colormap: &HashMap<char,Color>, keys: &Vec<char>, steps : usize) {
...
fn print_grid2(grid: &[Vec<char>], colormap: &HashMap<char,Color>, robot: usize, keys: &Vec<char>, steps: usize)
{
    let mut stdout = stdout();
    execute!(stdout, MoveTo(0, 0), Hide, Clear(ClearType::All));
    ...
    for k in keys.iter()
    {
        execute!(stdout, SetForegroundColor(*colormap.get(&k.to_ascii_uppercase()).unwrap()));
        print!("⚩ ");
    }
    stdout.flush();
