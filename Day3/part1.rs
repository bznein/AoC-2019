        ...
        match &wire_1[i][0..1] {
            "R" => {
                for j in start_x+1..=start_x +temp {
                    p1.x = j;
                    grid.insert(p1,Corner{wire_1 : true, wire_2:false});
                }
            }
            "U" => {
    ...
    for i in 0..wire_2.len() {
        ...
        match &wire_2[i][0..1] {
            "R" => {
                for j in start_x+1..=start_x +temp {
                    p1.x = j;
                    grid.entry(p1).and_modify(|c| c.wire_2 = true).or_insert(Corner{wire_1 : false, wire_2:true});
                }
            }
            "U" => {
                for j in start_y+1..=start_y + temp {
