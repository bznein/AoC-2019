...
fn move_direction(m: MoveDirection, dir: i32) -> MoveDirection
{
	match m 
	{
		UP => if dir==0 {LEFT} else {RIGHT},
		RIGHT => if dir==0 {UP} else {DOWN},
		DOWN => if dir==0 {RIGHT} else {LEFT},
		LEFT => if dir==0 {DOWN} else {UP},
	}
}
...
fn print_map(min_i : i32, min_j: i32, max_i: i32, max_j: i32, m : HashMap<(i32,i32),i64>)
{
	for i in min_i..=max_i
	{
		for j in min_j..=max_j
		{
			...
			{
				print!("{}", if *m.get(&(i,j)).unwrap()==1 {"█"} else {" "});
			}
			else 
			{
				print!(" ");
...
fn main() {
	...
	loop
	{
		...
		let new_val = output[0];
		m.insert((p.i,p.j),new_val);
		let move_val = output[1];
